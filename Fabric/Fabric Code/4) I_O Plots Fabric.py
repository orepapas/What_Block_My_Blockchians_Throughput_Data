...
figures_directory = 'What Blocks My Blockchain’s Throughput - Data/Fabric/Figures/I_O/'
...
peer_data_resources = load_data_for_frequencies_io(frequencies, 'peer', data_directory)
orderer_data_resources = load_data_for_frequencies_io(frequencies, 'orderer', data_directory)
...
peer_data_io_15s = process_io_data(peer_data_resources, 'peer', start_seconds, end_seconds)
orderer_data_io_15s = process_io_data(orderer_data_resources, 'orderer', start_seconds, end_seconds)


def plot_io_dot_plot(client_data_15s, peer_data_15s, orderer_data_15s, output_path):
    """
     ...
     - peer_data_15s: Filtered DataFrame containing I/O data for a specific peer in a 15 sec window.
     - orderer_data_15s: Filtered DataFrame containing I/O data for a specific orderer in a 15 sec window.
    ...
    io_usage_peer = peer_data_15s.groupby(['freq', 'peer', 'secs'])['io'].mean().reset_index()
    io_usage_orderer = orderer_data_15s.groupby(['freq', 'orderer', 'secs'])['io'].mean().reset_index()

    io_usage_client['type'] = 'client'
    io_usage_peer['type'] = 'peer'
    io_usage_orderer['type'] = 'orderer'

    io_usage_peer.rename(columns={'peer': 'client'}, inplace=True)
    io_usage_orderer.rename(columns={'orderer': 'client'}, inplace=True)

    # Concatenate the two DataFrames
    combined_data = pd.concat([io_usage_client, io_usage_peer, io_usage_orderer], ignore_index=True)

    # Now let's plot the data
    plt.figure(figsize=(8, 6))
    sns.scatterplot(data=combined_data, x='freq', y='io', hue='type', palette=['#bb6894', '#92aed0', '#29ad99'],
                    s=50, alpha=0.6, )
    ...
    plt.savefig(os.path.join(output_path, 'i_o_dot_plot.pdf'))
...
combined_io_data = plot_io_dot_plot(client_data_io_15s, peer_data_io_15s, orderer_data_io_15s, figures_directory)


def plot_mean_io_peer(peer_data_15s, output_path):
    """
    ...
    - output_path: The path where the output plot will be saved.
    """
    peer_data_15s = peer_data_15s[['freq', 'peer', 'io']]
    peer_data_15s['peer_id'] = peer_data_15s['peer'].str.extract('(\d+)').astype(int)
    mean_peer_io = peer_data_15s.groupby(['freq', 'peer_id'])['io'].mean().reset_index()
    pivoted_df_peer = mean_peer_io.pivot(index='freq', columns='peer_id', values='io')
    ...
    ensure_dir(os.path.dirname(output_path))
    plt.savefig(os.path.join(output_path, 'peer_mean_io_plot.pdf'))
    plt.show()


plot_mean_io_peer(peer_data_io_15s, figures_directory)


def plot_mean_io_orderer(orderer_data_15s, output_path):
    """
    Generate and save a line plot comparing average I/O utilization across different frequencies for orderers.

    Parameters:
    - orderer_data_15s: Filtered DataFrame containing memory data for a specific orderer in a 15 sec window.
    - output_path: The path where the output plot will be saved.
    """
    orderer_data_15s = orderer_data_15s[['freq', 'orderer', 'io']]
    orderer_data_15s['orderer_id'] = orderer_data_15s['orderer'].str.extract('(\d+)').astype(int)
    mean_orderer_io = orderer_data_15s.groupby(['freq', 'orderer_id'])['io'].mean().reset_index()
    pivoted_df_orderer = mean_orderer_io.pivot(index='freq', columns='orderer_id', values='io')
    ...
    ensure_dir(os.path.dirname(output_path))
    plt.savefig(os.path.join(output_path, 'orderer_mean_io_plot.pdf'))
    plt.show()


plot_mean_io_orderer(orderer_data_io_15s, figures_directory)
