...
peer_data_cpu_15s = process_node_data(frequencies, 'peer', 13, 27)
peer_data_cpu_15s_max = peer_data_cpu_15s.groupby(['time', 'peer', 'freq'])['%usage'].max().reset_index()
peer_data_cpu_15s_max['time'] = pd.to_datetime(peer_data_cpu_15s_max['time'], format='%H:%M:%S')
peer_data_cpu_15s_max = peer_data_cpu_15s_max.set_index('time')
peer_data_cpu_15s_max_3Sw = peer_data_cpu_15s_max.groupby(['peer', 'freq']).rolling('3S')['%usage'].sum()/3
peer_data_cpu_15s_max_3Sw = peer_data_cpu_15s_max_3Sw.reset_index()
peer_data_cpu_15s_max_3Sw['time'] = pd.to_datetime(peer_data_cpu_15s_max_3Sw['time'], format='%H:%M:%S').dt.time
peer_data_cpu_15s_max_3Sw = peer_data_cpu_15s_max_3Sw.groupby(['peer', 'freq']).apply(lambda x: x.iloc[2:]).reset_index(drop=True)
...
peer_data_network_15s = process_network_data(peer_data_network, 'peer', 13, 27)
peer_data_network_15s = peer_data_network_15s[['time', 'network_Mbps', 'peer', 'freq', 'secs']]
peer_data_network_15s['time'] = pd.to_datetime(peer_data_network_15s['time'], format='%H:%M:%S')
peer_data_network_15s = peer_data_network_15s.set_index('time')
peer_data_network_15s_3Sw = peer_data_network_15s.groupby(['peer', 'freq']).rolling('3S')['network_Mbps'].sum()/3
peer_data_network_15s_3Sw = peer_data_network_15s_3Sw.reset_index()
peer_data_network_15s_3Sw['time'] = pd.to_datetime(peer_data_network_15s_3Sw['time'], format='%H:%M:%S').dt.time
peer_data_network_15s_3Sw = peer_data_network_15s_3Sw.groupby(['peer', 'freq']).apply(lambda x: x.iloc[2:]).reset_index(drop=True)


def plot_response_cpu_dot_plot(txs_received, peer_cpu_data, output_path):
    """
     ...
     - peer_cpu_data: Filtered DataFrame containing CPU utilisation data of peers.
     - output_path: The path where the output plot will be saved.
     """
    resp_cpu = txs_received.merge(peer_cpu_data, left_on='time', right_on='time').drop('freq_y', axis=1)
    resp_cpu.columns = ['time', 'secs', 'freq', 'fresp', 'peer', '%usage']
    ...
    sns.scatterplot(data=resp_cpu, x='fresp', y='%usage', hue='freq', palette=custom_palette, s=50, alpha=0.6)

    plt.xlabel('f$_{resp}$', size=24)
    plt.ylabel('Max CPU (%)', size=26)
    plt.legend().remove()
    ...
    plt.savefig(os.path.join(output_path, 'plot_response_cpu_dot_plot.pdf'))

    plt.show()


plot_response_cpu_dot_plot(txs_received_per_second, peer_data_cpu_15s_max_3Sw, figures_directory)


def plot_response_network_dot_plot(txs_received, peer_network_data, output_path):
    """
     ...
     - output_path: The path where the output plot will be saved.
     """
    resp_network = txs_received.merge(peer_network_data, left_on='time', right_on='time').drop('freq_y', axis=1)
    resp_network.columns = ['time', 'secs', 'freq', 'fresp', 'peer', 'network_Mbps']
    ...
    plt.figure(figsize=(8, 6))
    sns.scatterplot(data=resp_network, x ='fresp', y ='network_Mbps' , hue='freq', palette=custom_palette, s=50, alpha=0.6)

    plt.xlabel('f$_{resp}$', size=24)
    plt.ylabel('Network (Mbps)', size=26)
    plt.legend().remove()
    ...
    # Save the plot
    ensure_dir(os.path.dirname(output_path))
    plt.savefig(os.path.join(output_path, 'plot_response_network_dot_plot.pdf'))

    plt.show()


plot_response_network_dot_plot(txs_received_per_second, peer_data_network_15s_3Sw, figures_directory)
